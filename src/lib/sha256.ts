 ...
 *
 * Implemented locally (instead of using the async WebCrypto API) so that game
 * rounds can be resolved synchronously and verified in the browser without a
 * round trip. This powers the transparent "provably fair" demo randomness that
 * the Fairness page documents.
 */
...
export function sha256Bytes(message: Uint8Array): Uint8Array {
  ...
  // 64-bit big-endian bit length (demo payloads are far below 2^32 bits).
...
/** HMAC-SHA256 with a UTF-8 key and message. Returns the raw 32-byte MAC. */
